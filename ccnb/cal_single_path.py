...
def showpathsenergy(p_e):
    ...
    plt.plot(x_new, y_smooth,linewidth=3, color ='k')
    ...
    plt.xlabel("Migration path(%) ", fontsize=35)  # X轴标签
    plt.ylabel("Energy (eV)", fontsize=35)  # Y轴标签
    plt.show()


if __name__ == "__main__":
    
    filename_BVSE = 'example\\icsd_421479\\icsd_421479-LPOCl.npy'
    filename_CIF = 'example\\icsd_421479\\icsd_421479-LPOCl.cif'
    ...
    endpoints = [[ 0.25000 , 0.48460 , 0.75000],
                 [0.25000 , 0.75000 , 0.48460 ],
                ]
    p =calpath(endpoints, struc,energy,count_images=9)
    #count_images 是每段路径插入点的个数
    print(p)  #输出坐标
    p_e = pathenergy(energy, struc, p)
    print(p_e)  #输出能量
    '''
    with open(filename_CIF.split(".")[0] + str(endpoints[0][0])+ str(endpoints[0][1])+ str(endpoints[0][2])+'_pathsmessage.txt', 'w') as f:
        for i in range(len(p_e)):
            f.write(str(p_e[i][0]) + '  '+ str(p_e[i][1]) + '\n ')
    struc1 = Structure.from_file(filename_CIF)
    for j in range(1, len(p) - 1):
        struc1.insert(0, 'He', p[j])
    struc1.to(fmt='CIF', filename=filename_CIF.split(".")[0] + 'path')
    xcoords=[]
    ycoords=[]
    for j in range(len(p_e)):
        xcoords.append(p_e[j][0])
    ...
    poly = np.polyfit(xcoords, ycoords, deg=10)  # 最小二乘法多项式拟合
    z = np.polyval(poly, xcoords)
    energy_nihe = np.zeros(shape=(len(xcoords), 2))
    for i in range(len(xcoords)):
        energy_nihe[i][1] = z[i]
        energy_nihe[i][0] = xcoords[i]
    print(energy_nihe)
    with open(filename_CIF.split(".")[0] + 'pathsmessage_nihe.txt', 'w') as f:
        for i in range(len(energy_nihe)):
            f.write(str(energy_nihe[i][0]) + '  '+ str(energy_nihe[i][1]) + '\n ')
    plt.figure(figsize=(8, 5))  # 创建绘图对象
    plt.plot(xcoords, z, linewidth=3)
    plt.scatter(xcoords, ycoords, color='k', marker='o')
    plt.xticks(fontsize=10)
    plt.yticks(fontsize=10)
    plt.xlabel("Reaction Coordinate ",fontsize=15)  # X轴标签
    plt.ylabel("BVSE site Energy", fontsize=15)  # Y轴标签
    plt.show() '''
    paths_2_cif(p, p_e, filename_CIF)
    showpathsenergy(p_e)
