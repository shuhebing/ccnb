...
def non_equivalent_paths_between_latticesite(filename_CIF, filename_BVSE, filename_CAVD, energythreshold, moveion='Li'):
...
def non_equivalent_paths_between_voids(filename_CIF, filename_BVSE, filename_CAVD, energythreshold, moveion='Li'):
    voids, channels = load_voids_channels_from_file(filename_CAVD)
    struc = load_struc(filename_CIF)
    energy = load_bvse_from_npy(filename_BVSE)
    mn = MigrationNetwork(struc, energy, voids, channels,filename_CIF, moveion=moveion, ismergecluster=True,
...
def configure_neb_packet(file_prefix, mep, moveion="Li"):
...
def all_cal(filename_cif,moveion='Li',valenceofmoveion=1,energythreshold=None):
    ...
    mep = non_equivalent_paths_between_latticesite(filename_cif, filename_BVSE, filename_CAVD,
                                                   energythreshold, moveion=moveion)
    configure_neb_packet(file_prefix, mep, moveion=moveion)


if __name__ == "__main__":
    #filename_CAVD = "example/icsd_246817/icsd_246817_origin.net"
    #filename_BVSE = "example/icsd_246817/icsd_246817.npy"
    filename_CIF = "example/LGPS/LGPS.cif"
    all_cal(filename_CIF, moveion='Li', valenceofmoveion=1, energythreshold=None)
