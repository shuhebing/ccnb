...
class vasp_inputs(object):
    ...
    def set_kpts(self):
        self.kpts = ((int(round(30/self.struc.lattice.a)),int(round(30/self.struc.lattice.b)),int(round(30/self.struc.lattice.c))),)

    def set_symbols(self):
        func = lambda x,y:x if y in x else x + [y]
        species = reduce(func, [[], ] + self.struc.species)
    ...
    def kpoints(self):
        kp = Kpoints(comment =u'Automatic mesh', kpts=self.kpts, style=Kpoints.supported_modes.Monkhorst)
    ...
    def lsf(self,neb):
        ...
        file.write('#BSUB -x\n'.encode('ascii'))
        file.write('ncpus=`cat $LSB_DJOB_HOSTFILE | wc -l ` \n'.encode('ascii'))
        file.write(('mpirun  -hostfile $LSB_DJOB_HOSTFILE  -np  ${ncpus} ' + self.vasp_path +'\n').encode('ascii'))
        file.close()
